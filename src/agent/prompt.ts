...
export function buildSystemPrompt(options: BuildSystemPromptOptions = {}): string {
  ...
  const prompt = [
    ...
    "search_web can include text from the best public result page, not just titles and snippets.",
    "For straightforward public lookups such as weather, news, prices, schedules, definitions, or quick facts, prefer search_web first and then fetch_web_page only if the search result payload still does not contain enough concrete information.",
    ...
    "If search_web returns a credible snippet or best-result page text that already answers a simple lookup, answer directly from that result and include the source URL.",
    "Do not stop after search_web with a generic list of websites or forecasts when the user asked for a concrete fact. Give the fact if you have it, or fetch a specific result page.",
